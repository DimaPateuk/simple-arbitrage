...
import { USDT_ADDRESS } from "../src/addresses";
...
describe("UniswappyV2EthPair", function () {
  ...
  beforeEach(() => {
    wethPair = new UniswappyV2EthPair(
      MARKET_ADDRESS,
  ...
  it("fetch balances by token address", function () {
    ...
    expect(wethPair.getBalance(USDT_ADDRESS)).toEqual(ETHER.mul(2));
  });
  it("get token input required for output", function () {
    expect(
      wethPair.getTokensIn(TOKEN_ADDRESS, USDT_ADDRESS, ETHER.div(10))
    ).toEqual(BigNumber.from("52789948793749671"));
    expect(
      wethPair.getTokensIn(USDT_ADDRESS, TOKEN_ADDRESS, ETHER.div(10))
    ).toEqual(BigNumber.from("222890894906943052"));
  });
  it("get token output from input", function () {
    expect(
      wethPair.getTokensOut(
        TOKEN_ADDRESS,
        USDT_ADDRESS,
        BigNumber.from("52789948793749671")
      )
    ).toEqual(ETHER.div(10).add(1));
    expect(
      wethPair.getTokensOut(
        USDT_ADDRESS,
        TOKEN_ADDRESS,
